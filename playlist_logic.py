...
DEFAULT_PROFILE = {
    ...
    "hype_min_energy": 7,
    "chill_max_energy": 3,
    "favorite_genre": "ambient", #rock --> ambient 
    "include_mixed": True,
}
...
def classify_song(song: Song, profile: Dict[str, object]) -> str:
    ...
    if energy >= hype_min_energy or genre == favorite_genre  or is_hype_keyword: # PART 2 BUG 1 FIX: switched order 
        return "Hype"
...
def compute_playlist_stats(playlists: PlaylistMap) -> Dict[str, object]:
    ...
    total = len(all_songs) #PART 2 BUG 2 FIX hype --> all_songs to compute the hype ratio correctly
    hype_ratio = len(hype) / total if total > 0 else 0.0

    avg_energy = 0.0
    if all_songs:
        total_energy = sum(song.get("energy", 0) for song in all_songs) #PART 2 BUG 2 FIX hype --> all songs to compute the average energy correctly
...
def search_songs(
    ...
    for song in songs:
        ...
        if value and q in value: #FIX PART 1:OG value in q which is incorrect bc the query can be a substr of the value and still match
